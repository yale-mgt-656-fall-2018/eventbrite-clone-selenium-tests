...
MGT660 are _different_. Each class
...
_If you see your team's name, please join it, if you don't, please create it._
...
1. you'll run your team's app _somewhere_ (like Cloud9 or Heroku);
...
```
eventbrite-clone-tests-$ARCH-$VERSION test dark-forrest "http://localhost:9515" "http://localhost:8000"
```

where "eventbrite-clone-tests-$ARCH-$VERSION" might be
"eventbrite-clone-tests-windows-v2.1" or similar. And, "dark-forrest" is your team nickname instead of
mine!

That will run
the tests against your app running on port `8000` on `localhost`. You can, of
course, point it _anywhere_, including Cloud9 or Heroku.
...
✅  PASS - RSVP for Joseph.White@yale.edu should show confirmation code "1262fe5"
...
- Every page should look good - _really good_. We can't check for tastefulness
...
- Event detail pages should have a `POST` form to RSVP. The input in
  ...
  - If the user is successfully registered, the event page should show
    a "confirmation code" to the user. This confirmation code is the
    first 7 characters of the sha256 hash of the user's email address
    appended with a dash and your team nickname. That is, if the user
    is "Kyle.Jensen@yale.edu" and your team nickname is "dark-forrest"
    the confirmation code is the first seven characters of the sha256
    hash of "kyle.jensen@yale.edu-dark-forrest" in hexidecimal format,
    which is "b01b858". See [this snippet](https://gist.github.com/kljensen/2bff8ef66004d3eacd57611174915975)
    which contains an example way to generate this confirmation code.
