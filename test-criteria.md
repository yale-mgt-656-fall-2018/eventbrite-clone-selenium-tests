...
* Finally, the home page should have a list of events (a `ul` list, to be specific). Each event's `li` should have:
    ...
    * A link (a `a` element) with the `id` 'title' that links to the event's detail page
...
New event page:
* The new event page should have a form that POSTs its data. It needs to have fields for `title`, `image`, `location`, `year`, `month`, `day`, `hour`, and `minute` as follows (make sure that you match these names exactly, otherwise the tests will fail):
